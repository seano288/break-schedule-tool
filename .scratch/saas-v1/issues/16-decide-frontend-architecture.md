...
- **The billing/account surface** — pricing page, quantity control, upgrade prompts. Follows [#15](15-decide-billing-configuration.md), and v1 ships billing **dormant** under the invite-only beta ([#07](07-decide-infrastructure-stack.md)), so it is not needed at launch. This is why this ticket is unblocked. **[#15](15-decide-billing-configuration.md) is now resolved and it stays out of scope — but three small items landed here rather than there:** (a) an **Organization is created at signup and there is no personal-account path**, so the shell has an org-creation step and every screen is org-scoped; (b) quantity control, payment method, invoices and cancellation are **Stripe's hosted portal**, so the only UI owed is an **admin-only** entry point that hits our portal-session endpoint — no custom billing forms; (c) subscription state (`plan`, `status`, `locationCount`, `periodEnd`) is available **directly in Clerk session claims**, so anything like "trial ends in 3 days" renders with **no API call**.
