...
import { setLocationHours } from '../lib/settingsService.js';

export async function locationsHoursHandler(request) {
    ...
    return runLocationSettingsAction(facade, target, () =>
        setLocationHours(facade, target, form)
    );
}

app.http('locationsHours', {
    methods: ['POST'],
    authLevel: 'anonymous',
    route: 'locations/hours',
    handler: locationsHoursHandler
});
