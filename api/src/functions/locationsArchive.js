...
import { archiveCompanyLocation } from '../lib/locationService.js';
import { runLocationAction } from '../lib/locationHttp.js';

export async function locationsArchiveHandler(request) {
    ...
    return runLocationAction(facade, link, () =>
        archiveCompanyLocation(facade, { companyId: link.companyId, locationId: form.get('id') })
    );
}

app.http('locationsArchive', {
    methods: ['POST'],
    authLevel: 'anonymous',
    route: 'locations/archive',
    handler: locationsArchiveHandler
});
