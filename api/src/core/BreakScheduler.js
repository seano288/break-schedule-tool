 ...
 * @param {Object} ruleset - Jurisdiction ruleset for meal/rest-break formulas
 * @returns {Map<string, EmployeeSchedule>}
 */
function parseScheduleRows(rows, dataStart, shiftCol, ruleset) {
    ...
    for (let i = dataStart; i < rows.length; i++) {
        ...
        if (!schedules.has(name)) {
            schedules.set(name, new EmployeeSchedule(name, ruleset));
        }
 ...
 * @param {Object}  [options.ruleset]       - Jurisdiction ruleset (see ./rulesets); defaults to california
...
export function scheduleBreaks(schedule, options = {}) {
    ...
    const employeeSchedules = parseScheduleRows(schedule, dataStart, shiftCol, ruleset);
